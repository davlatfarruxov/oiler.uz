...
export function EditServiceDialog({
  ...
  const fetchServiceData = async () => {
    try {
      ...
      // Map service data to form format
      setFormData({
        ...
        dueDate: service.dueDate ? new Date(service.dueDate).toISOString().split('T')[0] : ''
      })
    } catch (error) {
      console.error('Failed to load service:', error)
      toast.error('Xatolik', {
        description: 'Xizmat ma\'lumotlarini yuklashda xatolik yuz berdi'
      })
      onClose()
    } finally {
      setIsLoading(false)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error('Xatolik', {
        description: error.response?.data?.message || 'Xizmatni yangilashda xatolik yuz berdi'
      })
    } finally {
